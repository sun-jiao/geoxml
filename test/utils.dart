...
Gpx createGPXWithRte() {
  ...
  gpx.rtes = [
    Rte(
        ...
        rtepts: [Wpt(lat: 51.5, lon: -0.1167, name: 'London'), Wpt(lat: 48.8667, lon: 2.3333, name: 'Paris')]),
    Rte(
        name: 'route from Paris to Londan',
        rtepts: [Wpt(lat: 48.8667, lon: 2.3333, name: 'Paris'), Wpt(lat: 51.5, lon: -0.1167, name: 'London')])
  ];
...
Gpx createGPXWithTrk() {
  ...
  gpx.trks = [
    Trk(name: 'route from London to Paris', trksegs: [
      Trkseg(trkpts: [Wpt(lat: 51.5, lon: -0.1167, name: 'London'), Wpt(lat: 48.8667, lon: 2.3333, name: 'Paris')])
    ]),
...
void expectXml(String xml1, String xml2) {
  ...
  expect(xml1.replaceAll(regexp, ''), xml2.replaceAll(regexp, '') /*, reason: xml1 */);
}
